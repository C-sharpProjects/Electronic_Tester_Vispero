...
namespace TESTER
{
    public partial class Form1 : Form
    {
        ...
        private void Start_BT_Click(object sender, EventArgs e)
        {
            ...
            if (TextBoxSN.Text.Length != 11 && ComboBoxModel.Text == "PWA-1506-00")
            {
                do
                {
                    TextBoxSN.Text = "";
                    TextBoxSN.Text = Interaction.InputBox("Scan Serial Number ID (11 digit)", "TESTING :", "", 500, 450);
                    if (string.IsNullOrEmpty(TextBoxSN.Text))
                    {
                        stop_process();
                        return;
                    }
                } while (TextBoxSN.Text.Length != 11);
            }

            if (TextBoxSN.Text.Length != 10 && ComboBoxModel.Text == "PWA-1519-00")
            {
                do
                {
                    TextBoxSN.Text = "";
